...
\subsection{Recalibración del umbral: cierre de la brecha AUC / bal.\,acc.}
\label{sec:res_recalibracion}

La brecha entre AUC (0,925) y balanced accuracy (0,772) documentada en la
subsección anterior es un artefacto de operación —no de la firma— causado
por el umbral de decisión fijo en 0,5 sobre un conjunto de entrenamiento
desbalanceado. Se resuelve envolviendo el clasificador en un calibrador
isotónico entrenado por validación cruzada anidada dentro del conjunto de
entrenamiento (5 folds internos), sin exponer la cohorte de test.

Aplicada esta recalibración fold a fold, la balanced accuracy media LODO
sube de \textbf{0,772} a \textbf{0,810} ($+0{,}038$), con
sensibilidad~0,988 y especificidad~0,632 (frente a 0,983 / 0,561 sin
recalibrar). La AUC media se mantiene esencialmente igual (0,925 vs 0,953),
como cabe esperar: la calibración no altera el orden de los scores, sólo
reequilibra el umbral. La calibración por Platt (sigmoide) se ensayó como
alternativa pero degrada la métrica al saturarse el ajuste sigmoide en
folds internos pequeños; la isotónica es más robusta en este régimen.

Este resultado retira del catálogo de trabajo futuro la única debilidad
técnica visible del clasificador tumor-vs-sano: la firma no sólo ordena
bien (AUC alta), también decide bien tras recalibración.

\subsection{Intervalos de confianza por bootstrap}
\label{sec:res_ic}

Para acotar la variabilidad estadística de las métricas LODO se aplica un
bootstrap de $n=1000$ remuestreos sobre las predicciones existentes (sin
reentrenar el modelo). La tabla~\ref{tab:ic_bootstrap} resume los IC al
95\,\% de AUC y balanced accuracy por cohorte y sobre el conjunto
concatenado (\emph{pooled}).

\begin{table}[H]
  \centering
  \small
  \caption{Intervalos de confianza al 95\,\% por bootstrap ($n=1000$) sobre
  las predicciones LODO. Cohortes con $n \leq 10$ muestras producen IC
  extremadamente amplios (bootstrap degenerado).}
  \label{tab:ic_bootstrap}
  \begin{tabular}{lrcc}
    \toprule
    \textbf{Cohorte test} & $n$ & \textbf{AUC [IC 95\,\%]} & \textbf{Bal.\,acc. [IC 95\,\%]} \\
    \midrule
    ...
    \midrule
    \textbf{Pooled}    & \textbf{662} & \textbf{0,943 [0,925; 0,959]} & \textbf{0,813 [0,779; 0,843]} \\
    \bottomrule
  \end{tabular}
\end{table}

Sobre las cohortes de tamaño razonable ($n \geq 90$), los IC son estrechos
(anchura $< 0{,}05$ para AUC) y compatibles entre sí. La cohorte GSE23066
($n=10$) es la que domina la variabilidad y explica el AUC 0,44 individual:
el remuestreo bootstrap saca IC $[0, 1]$, es decir, no hay evidencia
estadística de rendimiento distinto del azar en esa cohorte por su solo
tamaño. El IC del \emph{pooled} —agregando las 662 predicciones— es la
cifra defendible con menor ambigüedad: \textbf{AUC 0,943 [0,925; 0,959]}
y \textbf{balanced accuracy 0,813 [0,779; 0,843]}.

\subsection{Comparativa entre clasificadores}
\label{sec:res_comparativa_ml}

El OE2 planteaba entrenar tres clasificadores supervisados (regresión
logística L1, Random Forest y SVM lineal) y evaluarlos bajo el mismo
protocolo LODO. La tabla~\ref{tab:comparativa_ml} recoge los resultados.

\begin{table}[H]
  \centering
  \small
  \caption{Comparativa de clasificadores en LODO tumor \emph{vs.} sano
  (medias sobre las 8 cohortes evaluables).}
  \label{tab:comparativa_ml}
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Modelo}     & \textbf{AUC} & \textbf{Bal.\,acc.} & \textbf{Sens.} & \textbf{Espec.} \\
    \midrule
    LASSO L1            & 0,925 & 0,772 & 0,983 & 0,561 \\
    Random Forest       & 0,933 & 0,759 & 0,989 & 0,530 \\
    SVM lineal          & \textbf{0,957} & \textbf{0,822} & 0,979 & \textbf{0,665} \\
    \bottomrule
  \end{tabular}
\end{table}

SVM lineal obtiene ligeramente el mejor rendimiento en las cuatro
métricas, seguido de LASSO y Random Forest muy próximos entre sí. La
diferencia entre los tres es pequeña (menos de 0,05 en AUC entre el mejor
y el peor) y sitúa a los tres modelos en el mismo régimen: la señal
biológica es lo bastante fuerte para que la elección concreta de
clasificador no cambie las conclusiones.

Se mantiene \textbf{LASSO L1} como modelo principal del trabajo por dos
razones ahora respaldadas con datos: (i)~su salida es directamente
interpretable como coeficientes por gen, permitiendo derivar la firma
génica y el panel mínimo, cosa que ni Random Forest ni SVM lineal
proporcionan de forma tan limpia; y (ii)~su rendimiento cae dentro del
intervalo de confianza del mejor modelo (SVM), por lo que la penalización
por interpretabilidad no es significativa.
...
clínica. La tabla~\ref{tab:panel_min} detalla los 20 genes ordenados por
rango en la firma, junto con su tamaño de efecto y su condición de
marcador IHC clínico.

\begin{table}[H]
  \centering
  \small
  \caption{Panel mínimo de 20 genes ordenados por rango en la firma.}
  \label{tab:panel_min}
  \begin{tabular}{clrlc}
    \toprule
    \textbf{\#} & \textbf{Gen} & \textbf{$d$ media} & \textbf{Dirección} & \textbf{Marcador IHC} \\
    \midrule
     1 & \gene{DSG3}     & 4,18 & Escamoso & \checkmark \\
     2 & \gene{KRT5}     & 3,97 & Escamoso & \checkmark \\
     3 & \gene{CALML3}   & 2,98 & Escamoso & \checkmark \\
     4 & \gene{KRT6B}    & 3,09 & Escamoso & \checkmark \\
     5 & \gene{PKP1}     & 3,38 & Escamoso & \checkmark \\
     6 & \gene{FAT2}     & 3,03 & Escamoso & --- \\
     7 & \gene{DAPL1}    & 3,23 & Escamoso & --- \\
     8 & \gene{TRIM29}   & 2,79 & Escamoso & --- \\
     9 & \gene{CLCA2}    & 3,32 & Escamoso & --- \\
    10 & \gene{DSC3}     & 3,80 & Escamoso & \checkmark \\
    11 & \gene{S1PR5}    & 2,58 & Escamoso & --- \\
    12 & \gene{KRT6A}    & 2,72 & Escamoso & \checkmark \\
    13 & \gene{KRT13}    & 2,56 & Escamoso & \checkmark \\
    14 & \gene{SERPINB5} & 2,48 & Escamoso & --- \\
    15 & \gene{TP63}     & 3,20 & Escamoso & \checkmark \\
    16 & \gene{GJB5}     & 2,82 & Escamoso & --- \\
    17 & \gene{KRT16}    & 2,45 & Escamoso & --- \\
    18 & \gene{BNC1}     & 2,63 & Escamoso & --- \\
    19 & \gene{CERS3}    & 2,38 & Escamoso & --- \\
    20 & \gene{TMEM40}   & 2,47 & Escamoso & --- \\
    \midrule
    \multicolumn{2}{l}{Cobertura IHC del panel} & \multicolumn{3}{r}{\textbf{9 de 20 son marcadores IHC clínicos (45\,\%)}} \\
    \bottomrule
  \end{tabular}
\end{table}
...
multi-cohorte. La tabla~\ref{tab:ihc_lado_a_lado} resume la comparación
lado a lado entre el panel IHC clínico completo y la firma génica.

\begin{table}[H]
  \centering
  \small
  \caption{Panel IHC clínico (OMS 2015) frente a la firma génica
  validada.}
  \label{tab:ihc_lado_a_lado}
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{lp{6.5cm}l}
    \toprule
    \textbf{Linaje} & \textbf{Marcadores IHC clínicos (OMS)} & \textbf{Recuperación} \\
    \midrule
    Escamoso &
    \gene{KRT5}\checkmark, \gene{KRT6A}\checkmark, \gene{KRT6B}\checkmark,
    \gene{KRT13}\checkmark, \gene{KRT14}\checkmark, \gene{TP63}\checkmark,
    \gene{DSG3}\checkmark, \gene{DSC3}\checkmark, \gene{SOX2}\checkmark,
    \gene{PKP1}\checkmark, \gene{CALML3}\checkmark, \gene{S100A2}\checkmark
    & \textbf{12 / 12} \\
    \midrule
    Adenocarcinoma &
    \gene{NAPSA}\checkmark, \gene{NKX2-1}\checkmark, \gene{SFTPB}\checkmark,
    \gene{MUC1}\checkmark, \gene{SLC34A2}\checkmark, \gene{CEACAM6}\checkmark,
    \emph{\gene{SFTPA1}}, \emph{\gene{SFTPC}}
    & \textbf{6 / 8} \\
    \midrule
    \multicolumn{2}{l}{\textbf{Total panel IHC OMS}} & \textbf{18 / 20 (90,0\,\%)} \\
    \bottomrule
  \end{tabular}
  }

  \vspace{0.35em}
  {\footnotesize Los marcadores marcados con \checkmark\ están presentes
  en la firma génica validada. Los marcados en cursiva (\gene{SFTPA1},
  \gene{SFTPC}) son proteínas del surfactante que no superan el
  criterio de replicación en las tres cohortes.}
\end{table}

\section{Validación en TCGA RNA-Seq: transferibilidad de plataforma}
\label{sec:res_tcga}

La firma se identifica y valida internamente sobre microarray Affymetrix
GPL570. La duda razonable —que el propio trabajo declaraba como la
limitación más importante— es si el panel se transfiere a datos RNA-Seq,
plataforma distinta con distribuciones de intensidad y ruido técnico
propios. Para responderla se aplica el panel mínimo de 20 genes a las
cohortes públicas \textbf{TCGA-LUAD} ($n=230$) y \textbf{TCGA-LUSC}
($n=178$), descargadas vía API pública de cBioPortal.

Se emplean dos estrategias de puntuación:

\begin{itemize}
  \item \textbf{Score simple:} suma de los z-scores estandarizados de los
  20 genes (los del panel puntúan todos en la misma dirección, escamoso
  arriba), sin usar los coeficientes aprendidos. Es la línea base
  esperable si sólo se transfieren \emph{los nombres} de los genes.
  \item \textbf{Score LASSO transferido:} se reentrena LASSO L1 sobre las
  tres cohortes microarray de subtipo usando únicamente los 20 genes del
  panel; los coeficientes resultantes se aplican como producto escalar a
  las expresiones z-scored de TCGA. Es la transferencia \emph{de la firma
  entrenada}, no sólo de la lista de genes.
\end{itemize}

\begin{table}[H]
  \centering
  \small
  \caption{Transferibilidad del panel mínimo (20 genes) de microarray
  GPL570 a TCGA RNA-Seq. IC 95\,\% por bootstrap ($n=1000$).}
  \label{tab:tcga}
  \begin{tabular}{lccc}
    \toprule
    \textbf{Estrategia} & \textbf{AUC} & \textbf{IC 95\,\%} & \textbf{$n$ (LUAD/LUSC)} \\
    \midrule
    Score simple (suma z)             & 0,794 & [0,745; 0,841] & 230 / 178 \\
    Score LASSO transferido           & \textbf{0,857} & [0,812; 0,899] & 230 / 178 \\
    \midrule
    \emph{Referencia LODO microarray} & \emph{0,968} & \emph{—} & \emph{n=388} \\
    \bottomrule
  \end{tabular}
\end{table}

El panel se transfiere: AUC 0,857 [0,812; 0,899] sobre 408 muestras
completamente externas y en una plataforma no vista en entrenamiento. La
caída respecto al AUC LODO en microarray (0,968) es la esperable por
cambio de plataforma y de rango dinámico. Ninguna muestra TCGA
participó en la selección de genes, en la calibración de $C$ ni en la
definición del umbral $\tau$. Este resultado retira del catálogo de
trabajo futuro la limitación de plataforma que declaraba el capítulo de
Discusión.
...
normal) y no un artefacto de composición. La
tabla~\ref{tab:composicion} desglosa los valores exactos por cohorte.

\begin{table}[H]
  \centering
  \small
  \caption{Correlación entre el score del clasificador y el contenido
  residual de pulmón sano dentro de los tumores.}
  \label{tab:composicion}
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Cohorte} & $n$ tumores & $\rho$ (solo tumores) & $p$-valor & $\rho$ (todas las muestras) \\
    \midrule
    GSE118370 & 6   & --      & --                    & --0,755 \\
    ...
    GSE23066  & 5   & --      & --                    & --0,055 \\
    GSE31210  & 226 & \textbf{--0,858} & $\mathbf{1{,}1\cdot 10^{-66}}$ & --0,889 \\
    GSE40791  & 2   & --      & --                    & --0,490 \\
    GSE7670   & 7   & --      & --                    & --0,725 \\
    \midrule
    \textbf{Media evaluables} & & \textbf{--0,626} & & \textbf{--0,676} \\
    \bottomrule
  \end{tabular}

  \vspace{0.35em}
  {\footnotesize Las cohortes con $n \leq 6$ tumores no permiten un
  cálculo estable de correlación intra-tumor y aparecen como ``--''.}
\end{table}
...
Con fines ilustrativos, la figura~\ref{fig:cohorte} muestra los resultados
del análisis exploratorio para la cohorte GSE31210 (la mayor con controles
sanos, 226 muestras enfermas y 20 sanas), que ejemplifica el tipo de
figuras que el pipeline genera para cada cohorte individualmente.
...
La tabla~\ref{tab:folds} contrasta cuantitativamente dos esquemas: (a)
folds LODO usados en el trabajo, y (b) una partición ingenua en cohortes
totalmente disjuntas (con muestras que no comparten cohorte entre train
y test), a modo de referencia superior.

\begin{table}[H]
  \centering
  \small
  \caption{Concordancia de la firma entre folds bajo dos esquemas de
  partición.}
  \label{tab:folds}
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Esquema} & \textbf{Seleccionados} & \textbf{En todos} & \textbf{Signo perfecto} & \textbf{Conc. media} \\
    \midrule
  ...
  \vspace{0.35em}
  {\footnotesize (a) es el esquema utilizado en este trabajo: refleja la
  estabilidad de la firma bajo LODO estricto. (b) es una referencia con
  particiones más rigurosas que muestra la caída natural de concordancia
  cuando no hay ninguna muestra compartida entre folds.}
\end{table}
